...
const manifestSchema = z.object({
  ...
  entrypoint: z.discriminatedUnion("type", [
    ...
    z.object({ type: z.literal("mcp-sse"), url: z.string().url(), headers: z.record(z.string()).optional() }),
    z.object({ type: z.literal("http"), url: z.string().url(), method: z.enum(["GET", "POST"]).optional() }),
...
export const POST = withErrors(async (request: Request) => {
  ...
  const skill = await skillRepository.create(input, caller.userId, scan.level);
