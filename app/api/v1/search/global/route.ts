/**
 * GET /api/v1/search/global?q=&source=github|synapth&tab=all|people|skills|skillsets&limit=  — the header search box:
...
import { GLOBAL_SEARCH_LIMIT, SEARCH_TABS, type GlobalSearchResponse } from "@/types/search";
...
const schema = z.object({
  ...
  tab: z.enum(SEARCH_TABS).default("all"),
  limit: z.coerce.number().int().min(1).max(30).default(GLOBAL_SEARCH_LIMIT),
});

export const GET = withErrors(async (request: Request) => {
  enforceRequestLimit("search", request);
  const { q, source, tab, limit } = schema.parse(Object.fromEntries(new URL(request.url).searchParams));
  const body: GlobalSearchResponse = { q, items: await globalSearch(q, { limit, source, tab }) };
