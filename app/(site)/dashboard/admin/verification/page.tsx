...
/** Admin panel · verification queue: open requests (oldest first) and recent decisions. Manual grants live in Users. */
export default async function VerificationQueuePage() {
  const session = await auth();
  if (!session?.user) redirect("/signin?callbackUrl=/dashboard/admin/verification");
...
function RequestList({ requests, empty, i18n }: { requests: VerificationRequest[]; empty: string; i18n: Translator<UiKey> }) {
  ...
  return (
    <ul className="stagger divide-y divide-border">
      {requests.map((r) => (
        <li key={r.id}>
          <Link href={`/dashboard/admin/verification/${r.id}`} className="group flex items-center gap-4 px-5 py-3.5 transition-colors hover:bg-accent/40">
