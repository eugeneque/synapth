...
/** Admin directory: find an account and change its role. */
export default async function UsersPage({ searchParams }: Props) {
  const session = await auth();
  if (!session?.user) redirect("/signin?callbackUrl=/dashboard/users");
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2 border-b border-border pb-6">
        <p className="label-mono-sm tracking-[0.2em] text-synapse">{t("users.crumb")}</p>
        <h1 className="font-display text-3xl font-medium tracking-tight sm:text-4xl">{t("users.title")}</h1>
        <p className="max-w-2xl text-sm text-muted-foreground">{t("users.lead")}</p>
      </header>

      <form className="flex flex-col gap-2 sm:flex-row" action="/dashboard/users">
