...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="container flex flex-col gap-8 py-8 lg:flex-row lg:items-start md:py-12">
      <ConsoleNav store={hasDatabase ? "postgres" : "in-memory"} handle={session.user.handle ?? "account"} role={await getRole(session.user.id)} className="w-full shrink-0 lg:sticky lg:top-24 lg:w-56 lg:min-h-[calc(100vh-8rem)]" />
