...
2. **Pixel / dot-matrix.** Единственный вид декора — точечная матрица и ASCII. Никаких градиентных блобов, стеклянных карточек и 3D. Всё, что «живёт», живёт на пиксельной сетке и реагирует на курсор. Единственное исключение — auth-gate (`/signin`, `/signup`, макет Stitch «Dynamic Glass Split Auth»): левая колонка — канвас с плазменными пятнами и решёткой частиц (`AuthCanvas`) и каскад матовых стеклянных панелей (`backdrop-blur`, `bg-white/[0.04]`), которые показывают реальные значения (режим, хранилище, локаль, правила сканера, цели установки).
3. **Чертёж.** Тонкие линии `border`, крестики-прицелы на углах (`Corners`), штриховка (`hatch`) как фон второстепенных зон. Радиусы мягкие, как на экранах Stitch: 6 px у контролов, 8 px у кнопок и инпутов, 12 px у карточек, 16 px у hero-панелей.
...
- Радиус: `--radius: 6px`. `rounded-md` = 6 px (чипы, бейджи), `rounded-lg` = 8 px (кнопки, инпуты, колодцы `.well`, вложенные плитки), `rounded-xl` = 12 px (карточки, панели `Panel`, `.frame`), `rounded-2xl` = 16 px (CTA-панель, auth-gate). `rounded-sm` = 2 px для ячеек heatmap. Круглыми остаются только пиксели/точки и аватары. Вложенные полосы (`.panel-head`, футер `Panel`) наследуют радиус через `rounded-t/b-[inherit]`.
