  ...
  ({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(
          "flex flex-1 items-center justify-between gap-4 py-4 text-left text-sm font-medium transition-colors hover:text-synapse [&[data-state=open]>svg]:rotate-45 [&[data-state=open]]:text-synapse",
          className,
        )}
        {...props}
      >
