...
export function CrawlPanel({ initial }: { initial: CrawlStatus }) {
  ...
  async function run(fn: () => Promise<CrawlStatus>) {
    ...
    try {
      ...
      setError(e instanceof AxonError ? e.message : "Request failed");
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="flex flex-col gap-5 p-5 text-sm">
      <div className="flex flex-wrap items-end gap-3">
        {status.running ? (
          ...
          <Button size="sm" className="font-mono text-[11px] uppercase tracking-[0.14em]" disabled={busy} onClick={() => run(() => axon.crawl.start({ maxRepos, minStars, repos: repos.trim() ? repos.split(",").map((r) => r.trim()).filter(Boolean) : undefined }))}>
            {busy ? <Loader2 className="animate-spin" /> : <Play />} [Run discovery crawl]
          </Button>
        )}
        <Button variant="mono" size="sm" aria-label="Refresh" onClick={() => run(() => axon.crawl.status())}>
          <RefreshCw /> [Refresh]
        </Button>
      ...
      <div className="grid gap-3 sm:grid-cols-[6rem_6rem_1fr]">
        <div className="space-y-1">
          <Label htmlFor="maxRepos" className="label-mono-sm">Repos</Label>
        ...
        <div className="space-y-1">
          <Label htmlFor="minStars" className="label-mono-sm">Min stars</Label>
        ...
        <div className="space-y-1">
          <Label htmlFor="repos" className="label-mono-sm">Specific repos (optional, comma-separated)</Label>
      ...
      <div className="well overflow-hidden">
        <div className="flex items-center justify-between border-b border-border bg-muted px-3 py-2">
          <span className="label-mono-sm inline-flex items-center gap-1.5">
            <span className={`inline-block h-2 w-2 rounded-full ${status.running ? "bg-synapse animate-pulse-dot" : "bg-border"}`} /> cortex-stream.stdout
          </span>
          <span className="label-mono-sm flex items-center gap-3">
            <span>encoding: utf-8</span>
            <span className="text-synapse">{log.length} events</span>
          </span>
        </div>
        <div className="h-56 overflow-y-auto p-4 font-mono text-xs leading-relaxed text-foreground">
          {log.length === 0 ? (
            <>
              <p className="text-muted-foreground">&gt; Crawler idle. Awaiting discovery vector…</p>
              {last && (
                <p className="text-muted-foreground">
                  &gt; Last run {last.finishedAt.replace("T", " ").slice(0, 16)} UTC: {last.processed} repos processed, +{last.created} created, ~{last.updated} updated.
                </p>
              )}
              {status.error && <p className="text-danger">&gt; Last error: {status.error}</p>}
            </>
          ) : (
            log.map((line, i) => (
        ...
        <div className="label-mono-sm flex items-center justify-between border-t border-border bg-surface px-3 py-1.5">
          <span className="inline-flex items-center gap-2">
            ...
            {p ? `${p.phase} · ${p.processed}/${p.discovered} · +${p.skillsCreated} new · ~${p.skillsUpdated} updated${p.current ? ` · ${p.current}` : ""}` : "listener: hooked on /api/v1/crawl"}
          </span>
          <span>{p ? `${pct}%` : `runtime: ${status.running ? "active" : "idle"}`}</span>
        </div>
        {p && (
      ...
      <div className="grid grid-cols-2 gap-3 text-center sm:grid-cols-4">
        <Stat label="Repos seen" value={status.state.repos} />
        <Stat label="Imported" value={status.state.imported} />
        <Stat label="Rejected" value={status.state.rejected} />
        <Stat label="Skills from GitHub" value={status.state.skills} accent />
      </div>
    </div>
  );
}
